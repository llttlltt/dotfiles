...
type InstallMethod = "vp" | "bun" | "pnpm" | "npm" | "brew" | "native";
...
async function detectInstallMethod(pi: ExtensionAPI): Promise<InstallMethod> {
  ...
  if (piPath?.includes("/.bun/") || realPiPath?.includes("/.bun/"))
    return "bun";
  if (
    piPath?.includes("/pnpm/") ||
    realPiPath?.includes("/pnpm/") ||
    piPath?.includes("/.pnpm/") ||
    realPiPath?.includes("/.pnpm/")
  )
    return "pnpm";
  ...
  switch (method) {
    ...
    case "pnpm":
      return {
        command: "pnpm",
        args: ["add", "-g", `${PACKAGE_NAME}@latest`],
        label: `pnpm add -g ${PACKAGE_NAME}@latest`,
      };
    case "npm":
      return {
        command: "npm",
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("update", {
    description:
      "Update Pi using vp, bun, npm, pnpm, brew, or native detection",
