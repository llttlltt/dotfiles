...
Commit messages for `wt step commit` are generated by a non-interactive Claude Code call using Haiku with tools disabled, so the Claude CLI must be installed and signed in.
