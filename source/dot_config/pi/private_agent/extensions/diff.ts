...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    if (changedFiles.size > 0) {
      ctx.ui.notify(
        `${changedFiles.size} changed file(s). Run /${commandName} to view/open in ${EDITOR.name}.`,
        "info",
      );
    }
  });
  ...
  pi.registerCommand(commandName, {
    description: `Show files changed by the last agent run and open one in ${EDITOR.name}`,
    handler: async (args, ctx) => {
      ...
      const selected = await ctx.ui.select(
        `Open changed file in ${EDITOR.name}`,
        labels,
      );
      if (!selected) return;
      ...
      const result = await pi.exec(EDITOR.command, ["-e", file], {
        cwd: ctx.cwd,
        timeout: 5000,
      });
      if (result.code === 0) {
        ctx.ui.notify(`Opened ${selected} in ${EDITOR.name}`, "info");
      } else {
        ctx.ui.notify(
          result.stderr.trim() ||
            `Failed to open ${selected} in ${EDITOR.name}`,
          "error",
        );
      }
    },
